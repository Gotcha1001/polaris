...
const TriangleMandalas2: React.FC = () => {
  ...
  const checkOverlap = (
    mandala1: PartialMandala,
    mandala2: PartialMandala
  ): boolean => {
    if (
      !mandala1.centerX ||
      !mandala1.centerY ||
  ...
  const findNonOverlappingPosition = (
    existingMandalas: (MandalaShape | PartialMandala)[],
    newMandala: PartialMandala,
  ...
  const generateMandalaShapes = (count: number): void => {
    ...
    for (let i = 0; i < count; i++) {
      ...
      const partialMandala: PartialMandala = { size };
      ...
      const colorPalette = Math.random();
      let color1: string, color2: string, extraColors: string[];
