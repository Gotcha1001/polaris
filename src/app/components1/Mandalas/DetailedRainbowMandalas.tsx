...
interface MandalaShape {
    ...
    primaryShapeType: string;
    secondaryShapeType: string;
    mixedShapeMode: number;
    delay: number;
    expiry: number;
    hasInnerCircle: boolean;
    hasOuterRing: boolean;
    hasDetailedCenter: boolean;
}
...
const complexShapeDefinitions: Record<string, ShapeDefinition> = {
...
const shapeTypes: string[] = Object.keys(complexShapeDefinitions);

const DetailedRainbowMandalas: React.FC = () => {
    ...
    const generateMandalaShapes = (): void => {
        const newMandalas: MandalaShape[] = Array(1).fill(0).map((_, i) => {
            ...
            const elements: number = Math.floor(Math.random() * 6) + 6; // 6-11 elements
            const rings: number = Math.floor(Math.random() * 3) + 3; // 3-5 rings

            // Create a color palette for this specific mandala
            const colorPalette: string[] = [];
            const startingColorIdx: number = Math.floor(Math.random() * rainbowColors.length);
            const colorRange: number = Math.floor(Math.random() * 3) + 3;
            for (let c = 0; c < colorRange; c++) {
                const idx: number = (startingColorIdx + c) % rainbowColors.length;
                colorPalette.push(rainbowColors[idx]);
            }

            const rotationSpeed: number = Math.random() > 0.5 ? 1 : -1; // Clockwise or counter-clockwise

            const primaryShapeType: string = shapeTypes[Math.floor(Math.random() * shapeTypes.length)];
            const secondaryShapeType: string = shapeTypes[Math.floor(Math.random() * shapeTypes.length)];

            const mixedShapeMode: number = Math.floor(Math.random() * 3); // 0: single shape, 1: alternate by ring, 2: mixed in each ring

            // Increment the key counter to ensure unique keys
            const uniqueKey: string = `mandala-${Date.now()}-${keyCounter}`;
    ...
    const renderShape = (
        ...
        const shape: ShapeDefinition | undefined = complexShapeDefinitions[shapeType];
        ...
        } else if (shape.type === "circle" && typeof shape.cx === 'number' && typeof shape.cy === 'number' && typeof shape.r === 'number') {
            return (
                <motion.circle
                    cx={shape.cx}
                    cy={shape.cy}
    ...
    const renderCompoundShape = (
        ...
        const primaryDef: ShapeDefinition | undefined = complexShapeDefinitions[primaryShape];
        const secondaryDef: ShapeDefinition | undefined = complexShapeDefinitions[secondaryShape];
                            ...
                            {Array.from({ length: mandala.rings }).map((_, ringIndex: number) => (
                                ...
                                >
                                    {Array.from({ length: mandala.elements }).map((_, elementIndex: number) => {
                                        const angle: number = (360 / mandala.elements) * elementIndex;
