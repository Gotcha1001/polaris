...
const DiamondMandalas: React.FC = () => {
                            ...
                            return (
                                ...
                                >
                                    {Array.from({ length: mandala.elements }).map((_, elementIndex) => {
                                        ...
                                        const diamondSize = ringSize / 3;
                                        const distance = ringSize / 2;
                                        const x = distance * Math.cos(angle);
                                        const y = distance * Math.sin(angle);
                                        const diamondColor = elementIndex % 2 === 0 ? mandala.color1 : mandala.color2;

                                        return (
                                            <motion.div
                                                key={`diamond-${ringIndex}-${elementIndex}`}
                                                className="absolute"
                                                style={{ left: 0, top: 0, width: 0, height: 0, transformOrigin: "center" }}
                                            ...
                                            >
                                                ...
                                                >
                                                    <motion.polygon
                                                        ...
                                                        fill={diamondColor}
                                                        stroke={diamondColor === mandala.color1 ? mandala.color2 : mandala.color1}
                                                        strokeWidth="2"
                                                        initial={{ opacity: 0 }}
                                                        animate={{ opacity: 1 }}
...
export default DiamondMandalas;