...
// Type definitions
type ColorScheme = "purple" | "blue" | "teal" | "custom";
...
interface TriangleMandalas3Props {
    maxMandalas?: number;
...
const TriangleMandalas3: React.FC<TriangleMandalas3Props> = ({
    maxMandalas = 5,
    ...
    const getRandomColors = useCallback((): { color1: string; color2: string } => {
    ...
    const Triangle = useCallback<React.FC<TriangleProps>>(
        ({
            size,
            color,
            strokeColor,
                ...
                {mandalaShapes.map((mandala) => {
                    ...
                    const easingType: Easing = 
                        mandala.fadeEffect === "back"
                            ? [0.68, -0.6, 0.32, 1.6] as [number, number, number, number]
