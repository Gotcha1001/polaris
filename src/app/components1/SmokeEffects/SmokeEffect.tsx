...
function SmokeEffect({
  isVisible,
}: SmokeEffectProps): React.ReactElement | null {
    ...
    const generateParticles = (
      ...
      return Array(count)
        ...
        .map((_, i) => {
          ...
          const whiteColor =
            smokeColors[
              Math.floor(seededRandom(seed + 3) * smokeColors.length)
            ];
          const purpleColor =
            purpleColors[
              Math.floor(seededRandom(seed + 4) * purpleColors.length)
            ];
          ...
          const mid1Y =
            (seededRandom(seed + 7) - 0.5) * (isSmall ? 60 : 100) -
            (isSmall ? 30 : 50);
          const mid2X =
            mid1X + (seededRandom(seed + 8) - 0.5) * (isSmall ? 40 : 80);
          const mid2Y =
            mid1Y +
            (seededRandom(seed + 9) - 0.5) * (isSmall ? 40 : 80) -
            (isSmall ? 30 : 50);
          const endX =
            mid2X + (seededRandom(seed + 10) - 0.5) * (isSmall ? 40 : 60);
          const endY =
            mid2Y +
            (seededRandom(seed + 11) - 0.5) * (isSmall ? 40 : 60) -
            (isSmall ? 30 : 50);
