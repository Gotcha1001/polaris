...
const paramsSchema = z.object({
  fileIds: z
    ...
    .min(1, "Provide at leas one file Id"),
});
...
export const createReadFilesTool = ({ internalKey }: ReadFilesToolOptions) => {
  return createTool({
    ...
    parameters: z.object({
      fileIds: z.array(z.string()).describe("Arra of file Ids to read"),
    }),
    handler: async (params, { step: toolStep }) => {
      ...
      try {
        return await toolStep?.run("read-files", async () => {
          ...
          if (results.length === 0) {
            return "Error: No files found with provided Id's. Use listFiles to get valid fileIDs.";
          }
