...
import { useProject } from "../../projects/hooks/use-projects";
...
export const ExportPopover = ({ projectId }: ExportGithubDialogProps) => {
  const project = useProject(projectId);
  const [open, setOpen] = React.useState(false);
  const { openUserProfile } = useClerk();

  const exportStatus = project?.exportStatus;
  const exportRepoUrl = project?.exportRepUrl;
    ...
    if (exportStatus === "failed") {
      return (
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm text-muted-foreground">Unable to export</p>
          <p className="text-xs text-muted-foreground text-center">
        ...
        <div className="space-y-5">
          <div className="space-y-1">
            ...
            <p className="text-sm text-muted-foreground">
              Export your project to a GitHub respository.
            </p>
          </div>
          ...
          >
            {([canSubmit, isSubmitting]) => (
              ...
              >
                {isSubmitting ? "Creating..." : "Create Respository"}
              </Button>
            )}
          </form.Subscribe>
        </div>
