...
// User for Agent to update conversation title
...
// User for Agent "CreateFolder" tool
...
export const deleteFile = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // Recursivley delete file/folder and all descendants
...
export const createBinaryFile = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const existing = files.find(
      (file) => file.name === args.name && args.name && file.type === "file",
    );
    if (existing) {
      throw new Error("File already exists");
    }
